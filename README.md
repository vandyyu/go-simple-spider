...
8. 前端页面交互的初步构想。
  - 可以手动新增种子，但是这么做，新的种子会存在新的解析规则和下载规则，所以应该要有相应的配置信息一起设置。可以考虑前端把定制的代码段或者代码文件上传到后端，后端动态监控，动态绑定代码文件, 这种应该比解析配置文件(解析配置文件需要定制很多逻辑，要考虑不同html风格适应问题，不够灵活)要简单得多。(通常情况下，后端已经把要抓取的网站固定好了，规则也做好了，然后启动了。但是考虑到运行途中要新增站点，所以应该需要这种设定。)
  - 可以使用前端网页查看爬虫的状态信息。
  - 可以在网页上实时配置，例如某些页面重试多次仍然失败了，此时可以通过手动配置一些信息，网后端发信息重新爬。
  - 下载功能。可选择已经爬好的页面，点击下载。
